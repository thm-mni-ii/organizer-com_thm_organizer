...
jQuery(document).ready(function ()
{
    ...
    function fixHeader()
    {
        ...
        {
            var consumptionTable = jQuery('.consumption-table');
            if (consumptionTable.scrollTop() >= 2)
            {
                consumptionTable.addClass('fixedTop');
                var _top = jQuery(this).scrollTop();
                jQuery('.fixedTop thead th').css('top', _top);
            }
            else
            {
                consumptionTable.removeClass('fixedTop');
            }
            if (consumptionTable.scrollLeft() >= 2)
            {
                consumptionTable.addClass('fixedLeft');
                var _left = jQuery(this).scrollLeft();
                jQuery('.fixedLeft tbody tr th:first-child').css('left', _left);
            }
            else
            {
                consumptionTable.removeClass('fixedLeft');
            }
        });
...
{
    var toggleImage = jQuery('#filter-toggle-image');
    if (toggleImage.hasClass("toggle-open"))
    {
        positionWhiteSpace();
    }
    if (toggleImage.hasClass("toggle-closed"))
    {
        positionWhiteSpace();
    }
});
...
{
    var consupmtionTable = jQuery('.consumption-table');
    if (consupmtionTable.length)
    {
        var offset = consupmtionTable.offset();
