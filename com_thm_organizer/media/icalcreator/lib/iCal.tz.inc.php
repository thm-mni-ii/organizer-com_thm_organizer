...
function expandTimezoneDates($vtzc)
{
    ...
    if (('-' != substr((string)$tzbefore['offsetSec'], 0, 1)) && ('+' != substr((string)$tzbefore['offsetSec'], 0,
                1))) {
    ...
    {
        $dtstart['unparsedtext'] = sprintf('%04d%02d%02dT%02d%02d%02d', $dtstart['year'], $dtstart['month'],
            $dtstart['day'], $dtstart['hour'], $dtstart['min'], $dtstart['sec']);
    }
    ...
    if (is_array($rrule)) {
        if ($rrule['FREQ'] == 'YEARLY') {
            ...
            while ($offsetchangetimestamp < $stoptimestamp && ($stopcount == 0 || $count < $stopcount)) {
                ...
                $offsetchangetimestamp = strtotime("+" . ((isset($rrule['INTERVAL']) && ($rrule['INTERVAL'] != 0)) ? $rrule['INTERVAL'] : 1) . " year",
