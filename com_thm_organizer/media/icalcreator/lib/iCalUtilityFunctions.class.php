...
class iCalUtilityFunctions
{
    ...
    private static $parValPrefix = [
        'MStz'   => ['utc-', 'utc+', 'gmt-', 'gmt+']
        ,
    ...
    public static function convEolChar(& $text, $nl)
    {
        ...
        /* fix empty lines */
        $text = str_replace(iCalUtilityFunctions::$baseDelim . iCalUtilityFunctions::$baseDelim,
            iCalUtilityFunctions::$baseDelim . str_pad('', 75) . iCalUtilityFunctions::$baseDelim, $text);
        /* fix line folding */
    ...
    public static function createTimezone(& $calendar, $timezone, $xProp = [], $from = null, $to = null)
    {
        ...
        foreach ($transitions as $tix => $trans) {                  // all transitions in date-time order!!
            ...
            if (!empty($prevOffsetfrom) || (0 == $prevOffsetfrom)) {
                ...
                $d             = explode('-',
                    $d);                   // set date to array to ease up dtstart and (opt) rdate setting
                $trans['time'] = [
                    'year'  => (int)$d[0],
                    'month' => (int)$d[1],
                    'day'   => (int)$d[2],
        ...
        if (empty($transTemp)) {      // if no match found
            if ($prevTrans) {            // then we use the last transition (before startdate) for the tz info
                ...
                $transTemp[0] = [
                    'time'       => $date->format(iCalUtilityFunctions::$fmt['YmdTHisO']),
...
//      $scomp->setProperty( 'x-utc-timestamp', $tix.' : '.$trans['ts'] );   // test ###
    ...
    public static function _date2strdate($datetime, $parno = 6)
    {
        ...
        $output .= 'T' . sprintf(iCalUtilityFunctions::$fmt['His'], $datetime['hour'], $datetime['min'],
                $datetime['sec']);
        if (isset($datetime['tz']) && ('' < trim($datetime['tz']))) {
            ...
            if ('Z' == $datetime['tz']) {
                ...
                try {
                    ...
                    $output = date('Ymd\THis',
                        mktime($datetime['hour'], $datetime['min'], ($datetime['sec'] - $offset), $datetime['month'],
    ...
    public static function _duration2date($startdate, $duration)
    {
        ...
        $date   = date(iCalUtilityFunctions::$fmt['YmdHis3'],
            mktime((int)$startdate['hour'], (int)$startdate['min'], (int)($startdate['sec'] + $dtend),
                (int)$startdate['month'], (int)$startdate['day'], (int)$startdate['year']));
        $d      = explode('-', $date);
    ...
    public static function _recur2date(& $result, $recur, $wdate, $fcnStart, $fcnEnd = false)
    {
        ...
        $fcnStartYMD = sprintf(iCalUtilityFunctions::$fmt['Ymd'], $fcnStart['year'], $fcnStart['month'],
            $fcnStart['day']);
        if (is_string($fcnEnd)) {
...
// echo 'recur='.str_replace( [ PHP_EOL, ' ' ], '', var_export( $recur, TRUE ))."<br> \n"; // test ###
        ...
        if (isset($recur['UNTIL'])) {
            ...
            if ($fcnEnd > $recur['UNTIL']) {
                ...
                $fcnEndYMD = sprintf(iCalUtilityFunctions::$fmt['Ymd'], $fcnEnd['year'], $fcnEnd['month'],
                    $fcnEnd['day']);
            }
            if (isset($recur['UNTIL']['hour'])) {
                $untilHis = sprintf(iCalUtilityFunctions::$fmt['His'], $recur['UNTIL']['hour'], $recur['UNTIL']['min'],
                    $recur['UNTIL']['sec']);
            } else {
...
// echo "bysetposXold_start=$bysetposYold $bysetposMold $bysetposDold<br>\n"; // test ###
            ...
            if ('YEARLY' == $recur['FREQ']) {
                ...
                $wdateYMD       = sprintf(iCalUtilityFunctions::$fmt['Ymd'], $wdate['year'], $wdate['month'],
                    $wdate['day']);
                iCalUtilityFunctions::_stepdate($fcnEnd, $fcnEndYMD,
                    ['year' => 1]); // make sure to count whole last year
            } elseif ('MONTHLY' == $recur['FREQ']) {
                $wdate['day'] = 1; // start from beginning of month
                $wdateYMD     = sprintf(iCalUtilityFunctions::$fmt['Ymd'], $wdate['year'], $wdate['month'],
                    $wdate['day']);
                iCalUtilityFunctions::_stepdate($fcnEnd, $fcnEndYMD,
                    ['month' => 1]); // make sure to count whole last month
            } else {
...
// echo 'output='.str_replace( [ PHP_EOL, ' ' ], '', var_export( $result, TRUE ))."<br> \n"; // test ###
        ...
        switch ($freq) {
            ...
            case 'WEEKLY':
                $intervalix = (int)date('W',
                    mktime(0, 0, $wkst, (int)$date['month'], (int)$date['day'], (int)$date['year']));
                break;
            case 'DAILY':
            default:
                ...
                if (iCalUtilityFunctions::_isArrayTimestampDate($rexrulevalue)) // timestamp, always date-time UTC
                {
                    ...
                    if ((3 < $parno) && isset($d['tz']) && ('Z' != $d['tz']) && iCalUtilityFunctions::_isOffset($d['tz'])) {
                        $strdate              = sprintf(iCalUtilityFunctions::$fmt['YmdHise'], (int)$d['year'],
                            (int)$d['month'], (int)$d['day'], (int)$d['hour'], (int)$d['min'], (int)$d['sec'],
                            $d['tz']);
    ...
    public static function _setDate2(
        ...
        if (iCalUtilityFunctions::_isArrayDate($year)) {
            ...
            if (!empty($tzid) && ('Z' != $tzid) && iCalUtilityFunctions::_isOffset($tzid)) {
                $d              = $input['value'];
                $strdate        = sprintf(iCalUtilityFunctions::$fmt['YmdHise'], (int)$d['year'], (int)$d['month'],
                    (int)$d['day'], (int)$d['hour'], (int)$d['min'], (int)$d['sec'], $tzid);
                $input['value'] = iCalUtilityFunctions::_strdate2date($strdate, 7);
                unset($input['value']['unparsedtext']);
            }
        } elseif (iCalUtilityFunctions::_isArrayTimestampDate($year)) {
    ...
    public static function _sortExdate1($a, $b)
    {
        ...
        $as .= (isset($a['hour'])) ? sprintf(iCalUtilityFunctions::$fmt['His'], (int)$a['hour'], (int)$a['min'],
            (int)$a['sec']) : '';
        $bs = sprintf(iCalUtilityFunctions::$fmt['His'], (int)$b['year'], (int)$b['month'], (int)$b['day']);
        $bs .= (isset($b['hour'])) ? sprintf(iCalUtilityFunctions::$fmt['His'], (int)$b['hour'], (int)$b['min'],
            (int)$b['sec']) : '';
    ...
    public static function _stepdate(& $date, & $dateYMD, $step = array('day' => 1))
    {
        ...
        $dateYMD = sprintf(iCalUtilityFunctions::$fmt['Ymd'], (int)$date['year'], (int)$date['month'],
            (int)$date['day']);
        unset($mcnt);
    }
    ...
    public static function _strdate2date($datetime, $parno = false, $wtz = null)
    {
        ...
        if (iCalUtilityFunctions::_isOffset(substr($datetime, -5, 5))) { // [+/-]NNNN offset
        ...
        } elseif (empty($wtz) && ctype_digit(substr($datetime, 0, 4)) && ctype_digit(substr($datetime, -2,
                2)) && iCalUtilityFunctions::_strDate2arr($datetime)) {
            $output = $datetime;
            if (!empty($tz)) {
                $output['tz'] = 'Z';
            }
            ...
            if ((ctype_digit(substr($datetime, 0, 8)) && ('T' == substr($datetime, 8,
                            1)) && ctype_digit(substr($datetime, -6, 6))) ||
