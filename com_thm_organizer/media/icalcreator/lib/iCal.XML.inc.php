        ...
        while (false !== ($subcomp = $component->getComponent())) {
            ...
            foreach ($subCompProps as $prop) {
                switch (strtolower($prop)) {
                    ...
                    case 'x-prop':
                        while (false !== ($content = $subcomp->getProperty($prop, false, true))) {
                            _addXMLchild($properties, $content[0], 'unknown', $content[1]['value'],
                                $content[1]['params']);
                        }
                        break;
...
function _addXMLchild(& $parent, $name, $type, $content, $params = [])
{
    ...
    if ((empty($content) && ('0' != $content)) || (!is_array($content) && ('-' != substr($content, 0,
                    1) && (0 > $content)))) {
        return;
    }
    /** store content */
    switch ($type) {
        ...
        case 'date-time':
            ...
            foreach ($content as $dt) {
                ...
                $str = sprintf($fmtYmdHis, (int)$dt['year'], (int)$dt['month'], (int)$dt['day'], (int)$dt['hour'],
                    (int)$dt['min'], (int)$dt['sec']);
        ...
        case 'geo':
            if (!empty($content)) {
                $v1 = $child->addChild('latitude',
                    iCalUtilityFunctions::_geo2str2($content['latitude'], iCalUtilityFunctions::$geoLatFmt));
                $v1 = $child->addChild('longitude',
                    iCalUtilityFunctions::_geo2str2($content['longitude'], iCalUtilityFunctions::$geoLongFmt));
            }
            break;
        case 'integer':
        ...
        case 'period':
            ...
            foreach ($content as $period) {
                ...
                $str = sprintf($fmtYmdHis, (int)$period[0]['year'], (int)$period[0]['month'], (int)$period[0]['day'],
                    (int)$period[0]['hour'], (int)$period[0]['min'], (int)$period[0]['sec']);
                if (isset($period[0]['tz']) && ('Z' == $period[0]['tz'])) {
                    $str .= 'Z';
                }
                $v2 = $v1->addChild('start', $str);
                if (array_key_exists('year', $period[1])) {
                    $str = sprintf($fmtYmdHis, (int)$period[1]['year'], (int)$period[1]['month'],
                        (int)$period[1]['day'], (int)$period[1]['hour'], (int)$period[1]['min'],
                        (int)$period[1]['sec']);
                ...
                switch ($rulelabel) {
                    case 'until':
                        if (isset($rulevalue['hour'])) {
                            $str = sprintf($fmtYmdHis, (int)$rulevalue['year'], (int)$rulevalue['month'],
                                    (int)$rulevalue['day'], (int)$rulevalue['hour'], (int)$rulevalue['min'],
                                    (int)$rulevalue['sec']) . 'Z';
                        } else {
                            $str = sprintf($fmtYmd, (int)$rulevalue['year'], (int)$rulevalue['month'],
                                (int)$rulevalue['day']);
                        }
