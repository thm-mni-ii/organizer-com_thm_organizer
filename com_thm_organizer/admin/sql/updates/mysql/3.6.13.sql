...
CREATE TABLE IF NOT EXISTS `#__thm_organizer_calendar_configuration_map` (
  ...
  CONSTRAINT `calendar_configuration_map_calendarID_fk` FOREIGN KEY (`calendarID`) REFERENCES `#__thm_organizer_calendar` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `calendar_configuration_map_configurationID_fk` FOREIGN KEY (`configurationID`) REFERENCES `#__thm_organizer_lesson_configurations` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE
)
  ENGINE = InnoDB
