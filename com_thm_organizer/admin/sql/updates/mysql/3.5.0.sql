...
SET `field_en` = `field_de`;

ALTER TABLE `#__thm_organizer_degrees`
  CHANGE `lsfDegree` `code` VARCHAR(10) DEFAULT '';
...
ALTER TABLE `#__thm_organizer_rooms`
  ADD `capacity` INT(4) UNSIGNED DEFAULT NULL;

ALTER TABLE `#__thm_organizer_room_types`
  MODIFY `type` VARCHAR(150) NOT NULL,
  MODIFY `gpuntisID` VARCHAR(60) NOT NULL DEFAULT '';

UPDATE `#__thm_organizer_room_types`
SET `type` = CONCAT(`type`, ', ', `subtype`);
...
CREATE TABLE IF NOT EXISTS `#__thm_organizer_methods` (
  `id`              INT(11) UNSIGNED NOT NULL AUTO_INCREMENT,
  `untisID`         VARCHAR(60)      NOT NULL DEFAULT ''
  COMMENT 'The Untis internal ID',
...
REFERENCES `#__thm_organizer_room_types` (`id`)
  ON DELETE SET NULL
  ON UPDATE CASCADE;
...
REFERENCES `#__thm_organizer_departments` (`id`)
  ON DELETE CASCADE
  ON UPDATE CASCADE;
...
REFERENCES `#__thm_organizer_lesson_subjects` (`id`)
  ON DELETE CASCADE
  ON UPDATE CASCADE;
