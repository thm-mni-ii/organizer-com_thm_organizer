...
/**
 * Spezieller XmlReader zum Einlesen der StundenplanRohDaten
 */
SchedXmlReader = function() {
	...
	SchedXmlReader.superclass.constructor.call(this, meta, recordType
			|| meta.fields);
}
Ext
		.extend(
				...
				Ext.data.XmlReader,
				{
					extractValue : function(n, f) {
						...
						n = n[0] ? n[0] : n;
						var v = (n && n.firstChild ? n.firstChild.nodeValue
								: null);
					...
					readRecord : function(root, def, key, records) {
						...
						for ( var j = 0, jlen = fields.length; j < jlen; j++) {
							...
							// Hier werden, falls vorhanden. auch mehrere
							// Kindelemente als Array gespeichert
							var arr = q.select(f.mapping, n);
							var val = [];
							var number = q.selectValue("periods", n);
							if ((f.name == 'room' || f.name == 'dow' || f.name == 'block')
									&& arr.length != number) {
								...
								for ( var d = 0, dlen = arr.length; d < dlen; d++) {
									if (f.name == 'room' || f.name == 'clas'
											|| f.name == 'doz') {
										val[d] = arr[d].getAttribute('id');
									} else
										val[d] = this.extractValue(arr[d], f);
								}
							}
							if (f.xmlType == 'array') {
								values[f.name] = val;
						...
						for ( var i = 0; i < mydowarray.length; i++) {
							...
							// Berechnet Wochentag
							// values.dow =
							// this.numbertoday(parseInt(values.begin));
							// Erstellt eindeutigen Key
							values.key = (def.subject + "_" + values.dow + "_"
									+ values.block + "_" + lessonid)
									.toLowerCase();
							// Sporatische Veranstaltungen bekommen eine extra
							// ID
							if (def.type == 'sporadic')
								values.key = 'SP_' + values.key;
							...
							if (records[values.key.toString()]) {
								if (records[values.key.toString()].data.doz
										.search(new RegExp(values.doz)) == -1)
									values.doz = records[values.key.toString()].data.doz
											+ " " + values.doz;
								else
									values.doz = records[values.key.toString()].data.doz;

								if (records[values.key.toString()].data.room
										.search(new RegExp(values.room)) == -1)
									values.room = records[values.key.toString()].data.room
											+ " " + values.room;
								else
									values.room = records[values.key.toString()].data.room;
					...
					readRecords : function(doc) {
						...
						MySched.Mapping.load(Ext.DomQuery.select('document',
								doc));
						// Auslesen des Datums - Leider nicht nach ISO8601,
						// deshalb substring
						this.version = Ext.Date.parseDate(Ext.DomQuery
								.selectNode('document', doc).getAttribute(
										'date'), 'Ymd');
						this.sessionbegin = Ext.DomQuery.selectValue(
								'document>general>schoolyearbegindate', doc);
						this.sessionend = Ext.DomQuery.selectValue(
								'document>general>schoolyearenddate', doc);
						...
						var timeperiods = Ext.DomQuery.selectNode(
								'document>timeperiods', doc);

						if (timeperiods.children)
							timeperiods = timeperiods.children;
						else
							timeperiods = timeperiods.childNodes;
						for ( var tsi = 0; tsi < timeperiods.length; tsi++) {
							if (timeperiods[tsi].nodeType == 1) {
								if (timeperiods[tsi].children)
									var timeperiod = timeperiods[tsi].children;
								else
									var timeperiod = timeperiods[tsi].childNodes;
						...
						if (this.meta.totalRecords) {
							totalRecords = q.selectNumber(
									this.meta.totalRecords, root, 0);
						}

						if (this.meta.success) {
							var sv = q.selectValue(this.meta.success, root,
									true);
						...
						for ( var i = 0, len = ns.length; i < len; i++) {
							var lec = ns[i];
							var subject = q.selectNode('lesson_subject', lec)
									.getAttribute('id');
...
/**
 * Spzieller JsonReader fuer Laden/Speichern des Stundenplans
 * 
 * @author thorsten
 */
