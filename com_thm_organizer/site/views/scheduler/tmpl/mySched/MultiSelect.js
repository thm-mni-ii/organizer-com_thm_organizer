...
Ext.ux.form.MultiSelect = Ext.extend(Ext.form.Field, {
	...
	onRender : function(ct, position) {
		...
		var fs = this.fs = Ext.create('Ext.form.FieldSet', {
			renderTo : this.el,
			title : this.legend,
			height : this.height,
			width : this.width,
		...
		this.view = Ext.create('Ext.ListView', {
			multiSelect : true,
			store : this.store,
			columns : [ {
				header : 'Value',
	...
	getValue : function(valueField) {
		...
		for ( var i = 0; i < selectionsArray.length; i++) {
			returnArray.push(this.store.getAt(selectionsArray[i]).get(
					(valueField != null) ? valueField : this.valueField));
		}
...
Ext
		.extend(
				Ext.ux.form.MultiSelect.DropZone,
				Ext.dd.DropZone,
				{
					/**
					...
					onNodeDrop : function(n, dd, e, data) {
						...
						var insertAt = (this.ms.appendOnly || (n == this.ms.fs.body.dom)) ? this.view.store
								.getCount()
								: this.view.indexOf(n);
						if (pt == "below") {
							insertAt++;
						...
						if (data.sourceView == this.view) {
							...
							if (insertAt > this.view.store
									.indexOf(data.records[0])) {
								dir = 'down';
								insertAt--;
							}
						}
						...
						for ( var i = 0; i < data.records.length; i++) {
							...
							this.view.store.insert(dir == 'down' ? insertAt
									: insertAt++, r);
