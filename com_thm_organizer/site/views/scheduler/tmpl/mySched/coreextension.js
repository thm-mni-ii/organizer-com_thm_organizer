		...
		.override(
				Ext.picker.Date,
				{
					fullUpdate : function(date, active) {
						var me = this, cells = me.cells.elements, textNodes = me.textNodes, disabledCls = me.disabledCellCls, eDate = Ext.Date, i = 0, extraDays = 0, visible = me
								.isVisible(), sel = +eDate
								.clearTime(date, true), today = +eDate
								.clearTime(new Date()), min = me.minDate ? eDate
								.clearTime(me.minDate, true)
								: Number.NEGATIVE_INFINITY, max = me.maxDate ? eDate
								.clearTime(me.maxDate, true)
								: Number.POSITIVE_INFINITY, ddMatch = me.disabledDatesRE, ddText = me.disabledDatesText, ddays = me.disabledDays ? me.disabledDays
								.join('')
								: false, ddaysText = me.disabledDaysText, format = me.format, days = eDate
								.getDaysInMonth(date), firstOfMonth = eDate
								.getFirstDateOfMonth(date), startingPos = firstOfMonth
								.getDay()
								- me.startDay, previousMonth = eDate.add(date,
								eDate.MONTH, -1), longDayFormat = me.longDayFormat, prevStart, current, disableToday, tempDate, setCellClass, html, cls, formatValue, value;
						...
						days += startingPos;
						prevStart = eDate.getDaysInMonth(previousMonth)
								- startingPos;
						...
						if (me.showToday) {
							...
							disableToday = (tempDate < min
									|| tempDate > max
									|| (ddMatch && format && ddMatch.test(eDate
											.dateFormat(tempDate, format))) || (ddays && ddays
						...
						setCellClass = function(cell) {
							...
							if (current >= begin && current <= end) {
								...
								if (!cell.children[0].className
										.contains(" calendar_tooltip"))
...
calendar_tooltip = function(e) {
		...
		if (Ext.isObject(events[i])) {
			...
			for ( var eventIndex = 0; eventIndex < eventObjects.length; eventIndex++) {
				...
				if (o.type === "teacher")
					...
					name += "<small class='roomshortname'>" + o.name
							+ "</small>";
				else
					name += o.name
			}
	...
	if (events.length > 0) {
		...
		var ttInfo = Ext.create('Ext.tip.ToolTip', {
			title : '<div class="mySched_tooltip_calendar_title">'
					+ MySchedLanguage.COM_THM_ORGANIZER_SCHEDULER_EVENTS
					+ '</div>',
