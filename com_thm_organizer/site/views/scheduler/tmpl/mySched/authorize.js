...
MySched.Authorize = function() {
	...
	return {
							...
							try {
								...
								if (json.success) {
									...
									if (json.errors.reason) {
										MySched.CookieProvider
												.clear('authToken');
										Ext.Msg
												.alert(
														MySchedLanguage.COM_THM_ORGANIZER_SCHEDULER_AUTHORIZE_FAILED,
														MySchedLanguage.COM_THM_ORGANIZER_SCHEDULER_AUTHORIZE_FAILED_MSG1
																+ json.errors.reason
																+ MySchedLanguage.COM_THM_ORGANIZER_SCHEDULER_AUTHORIZE_FAILED_MSG2,
														this.showAuthForm(),
														this);
									}
		...
		checkAccess : function(type, id) {
			...
			if (this.accArr.ALL[type]) {
				if (this.accArr.ALL[type] === '*'
						|| this.accArr.ALL[type].indexOf(id) !== -1) {
					return true;
				}
			}
			...
			if (this.accArr[this.role][type]) {
				if (this.accArr[this.role][type] === '*'
						|| this.accArr[this.role][type].indexOf(id) !== -1) {
					return true;
				}
			}
			...
			if (this.additionalRights[type]) {
				if (this.additionalRights[type] === '*'
						|| this.additionalRights[type].indexOf(id) !== -1) {
					return true;
				}
			}
