	...
	return {
		...
		startMySched : function(json) {
			...
			for ( var i = 1; i <= length; i++) {
				...
				if (!MySched.daytime[json[i].day][json[i].period])
					MySched.daytime[json[i].day][json[i].period] = new Array();
				MySched.daytime[json[i].day][json[i].period]["etime"] = json[i].endtime
						.substr(0, 5);
		...
		afterLoad : function(ret) {
			...
			if (checkStartup("Events.load") === true) {
				MySched.TreeManager
						.afterloadEvents(MySched.startup["Events.load"].data);
			...
			if (Ext.isObject(jsonData)) {
				...
				if (MySched.SessionId) {
					...
					MySched.delta.load(_C('ajaxHandler'), 'json',
							MySched.delta.loadsavedLectures, MySched.delta,
							"delta");
				} else {
		...
		createUserSchedule : function() {
			if (!MySched.layout.tabpanel.getComponent('mySchedule')) {
				...
				var dropTarget = new Ext.dd.DropTarget(tabID, this
						.getDropConfig());
			}
		},
		/**
											...
											success : function(response) {
												try {
													...
													for ( var lectureIndex = 0; lectureIndex < lectures.length; lectureIndex++) {
														MySched.Schedule
																.addLecture(lectures[lectureIndex]);
													}

													/*
													 * Ext.each(s.getLectures(),
													 * function (e) {
...
//													MySched.loadedLessons[nodeID] = true;
								...
								if (typeof n.elements != "undefined")
									var s = new mSchedule(nodeID,
											'_tmpSchedule').init(type,
											n.elements);
								else
									var s = new mSchedule(nodeID,
											'_tmpSchedule').init(type, nodeKey);
						...
						MySched.Schedule.addLecture(MySched.Base.schedule
								.getLecture(data.id));
						MySched.selectedSchedule.eventsloaded = null;
						MySched.selectedSchedule.refreshView();
					}
		...
		freeBusyHandler : function(e, state) {
			if (!state) {
				Ext.select('.blockFree').replaceClass('blockFree',
						'blockFree_DIS');
				Ext.select('.blockBusy').replaceClass('blockBusy',
						'blockBusy_DIS');
				Ext.select('.blockOccupied').replaceClass('blockOccupied',
						'blockOccupied_DIS');
			} else {
				Ext.select('.blockFree_DIS').replaceClass('blockFree_DIS',
						'blockFree');
				Ext.select('.blockBusy_DIS').replaceClass('blockBusy_DIS',
						'blockBusy');
				Ext.select('.blockOccupied_DIS').replaceClass(
						'blockOccupied_DIS', 'blockOccupied');
			}
...
MySched.InfoPanel = function() {
	...
	return {
		...
		showInfo : function(el) {
			...
			if (!text)
				this.el
						.update(MySchedLanguage.COM_THM_ORGANIZER_SCHEDULER_LESSON_ERROR);
			else {
				this.el.update(text);
			}
		...
		updateDetailInfoClickHandler : function() {
			this.el.select('.detailInfoBtn').on('click', this.detailInfoClick,
					this);
		},
		/**
		...
		.apply(
				new Ext.util.Observable(),
				{
					...
					stopSelection : function(o) {
						if (Ext.type(o) == 'object') { // Nur unterhalb
														// uebergebenen Objekt
							...
							Ext.select('.status_icons_estudy')
									.removeAllListeners();
						} else if (MySched.layout.tabpanel.items.getCount() > 0) { // Nur
																					// Aktiven
																					// Tab
							var activeTabDom = MySched.layout.tabpanel
									.getActiveTab().getEl().dom;
					...
					startSelection : function(el) {
						var tab = el
								|| MySched.layout.tabpanel.getActiveTab()
										.getEl();
						if (!tab)
							return;
					...
					showSchedule : function(e, type) {
						var name = e.target.firstChild.nodeValue.replace(
								/^\s+/, '').replace(/\s+$/, '');
						...
						if (lessonid[1]) {
							...
							if (type == "teacher") {
								...
								for ( var i = 0; i < l.room.keys.length; i++) {
									var room = MySched.Mapping.getRoomName(
											l.room.keys[i]).split("/");
									if (name == room[room.length - 1].replace(
											/^\s+/, '').replace(/\s+$/, '')) {
							...
							nodeID = l.semesterID + "." + l.plantypeID + "."
									+ type + "." + parent + "." + nodeKey;
							...
							if (type == "teacher") {
								...
								for ( var i = 0; i < MySched.Mapping.room.keys.length; i++) {
									if (name == MySched.Mapping
											.getRoomName(MySched.Mapping.room.keys[i])) {
										nodeKey = MySched.Mapping.room.keys[i];
										gpuntisID = MySched.Mapping
												.getObjectField(
														type,
														MySched.Mapping.room.keys[i],
														"gpuntisID");
										parent = MySched.Mapping
												.getObjectField(
														type,
							...
							nodeID = semesterID + "." + plantypeID + "." + type
									+ "." + parent + "." + nodeKey;
						}
					...
					showEventInformation : function(e) {
						...
						var ttInfo = Ext.create('Ext.tip.ToolTip', {
							title : '<div class="MySchedEvent_tooltip"> '
									+ l.data.title + " " + '</div>',
						...
						var modulewin = Ext
								.create(
										'Ext.Window',
										{
											...
											hideLabel : true,
											closeable : true,
											html : '<iframe id="iframeModule" class="mysched_iframeModule" src="'
													+ externLinks.curriculumLink
													+ '&nrmni='
													+ l.moduleID.toUpperCase()
													+ '"></iframe>'
										});
					...
					showInformation : function(e) {
						...
						var ttInfo = Ext.create('Ext.tip.ToolTip', {
							title : '<div class="mySched_lesson_tooltip"> '
									+ l.data.desc + " " + '</div>',
							id : 'content-anchor-tip',
							target : el.id,
							anchorToTarget : true,
					...
					onMouseOver : function(e) {
						...
						if (el.id.substr(0, 4) != "delta"
							...
							this.selectButton.show().alignTo(el, 'tr-tr',
									[ -4, 5 ]);
						}
					},
					/**
					...
					lecture2ScheduleHandler : function() {
						...
						if (el.id.substr(0, 4) != "delta"
							...
							if (el.hasCls('lectureBox_cho')
									|| id.split('##')[0] == 'mySchedule') {
								if (typeof MySched.Base.getLecture(id) != "undefined")
									MySched.Schedule.removeLecture(MySched.Base
											.getLecture(id));
								else
									MySched.Schedule
											.removeLecture(MySched.Schedule
													.getLecture(id));
								...
								MySched.Schedule.addLecture(MySched.Base
										.getLecture(id));
					...
					deleteLesson : function(id) {
						...
						var tab = MySched.layout.tabpanel
								.getComponent(MySched.Base.schedule
										.getLecture(id).data.responsible);
						if (tab)
							tab.mSchedule.removeLecture(tab.mSchedule
									.getLecture(id));
						MySched.selectedSchedule
								.removeLecture(MySched.selectedSchedule
										.getLecture(id));
						MySched.Schedule.removeLecture(MySched.Schedule
								.getLecture(id));
						MySched.responsibleChanges
								.removeLecture(MySched.responsibleChanges
										.getLecture(id));
						MySched.Base.schedule
								.removeLecture(MySched.Base.schedule
										.getLecture(id));
					...
					onMouseDown : function(e) {
						var el = e.getTarget('.lectureBox', 5, true)
						if (el == null)
							return; // Element ist schon selektiert
						// Selektiere Element
		...
		if (tabs[i].mSchedule.status == "unsaved") {
			...
			Ext.Msg
					.show({
						...
						msg : text
								+ "<br/>"
								+ MySchedLanguage.COM_THM_ORGANIZER_SCHEDULER_CHANGES_SAVE,
						width : 400,
						modal : true,
								...
								for ( var ti = 0; ti < temptabs.length; ti++) {
									if (temptabs[ti].mSchedule.status == "unsaved") {
										if (temptabs[ti].mSchedule.id == "mySchedule")
											temptabs[ti].mSchedule.save(
													_C('ajaxHandler'), false,
													"UserSchedule.save");
										else
											temptabs[ti].mSchedule.save(
													_C('ajaxHandler'), false,
													"ScheduleChanges.save");
									}
								}
							}
...
function showLessonMenu(e) {
	...
	if (((lesson.data.owner == MySched.Authorize.user || MySched.Authorize
			.isClassSemesterAuthor()) && lesson.data.owner != null)
			&& lesson.data.owner != "gpuntis") {
...
//															MySched.TreeManager.add(record);
		...
		showInfoWindow : function() {
			if (Ext.ComponentMgr.get("infoWindow") == null
					|| typeof Ext.ComponentMgr.get("infoWindow") == "undefined") {
						...
						.create(
								'Ext.Window',
								{
									...
									html : "<small style='float:right; font-style:italic;'>Version "
											...
											+ "<li style='padding-left:10px;'>GE&Auml;NDERT: Informationen zu Veranstaltungen &uuml;ber Veranstaltungstitel (Klick auf den Titel).</li>"
											+ "</ul>"
											+ "<br/>"
											+ "<b>Version: 2.1.5:</b>"
											+ "<ul>"
											+ "<li style='padding-left:10px;'>NEU: Pers&ouml;nliche Termine k&ouml;nnen &uuml;ber den Men&uuml;punkt 'Neuer Termin' oder per Klick in einen Block angelegt werden.</li>"
											+ "<li style='padding-left:10px;'>NEU: Berechtigte Benutzer d&uuml;rfen im Panel 'Einzel Termine' neue Termine anlegen oder alte editieren.</li>"
											+ "<li style='padding-left:10px;'>GE&Auml;NDERT: &Auml;nderungen werden wie ein Stundenplan aufgerufen.</li>"
											+ "<li style='padding-left:10px;'>GE&Auml;NDERT: Nur registrierte Benutzer haben Zugriff auf alle Funktionen.</li>"
											+ "</ul>"
											+ "<br/>"
											+ "<b>Version: 2.1.4:</b>"
											+ "<li style='padding-left:10px;'>NEU: In der Infoanzeige von Veranstaltungen kann die Modulbeschreibung abgerufen werden.</li>"
											+ "<li style='padding-left:10px;'>GE&Auml;NDERT: Termine werden nur noch an betroffenen Tagen angezeigt.</li>"
											+ "<li style='padding-left:10px;'>GE&Auml;NDERT: Termine werden bei Klick auf das orangene Ausrufezeichen angezeigt.</li>"
											+ "<br/>"
											+ "<b>Version: 2.1.3:</b>"
											+ ""
											+ "<li style='padding-left:10px;'>NEU: Stundenpl&auml;ne k&ouml;nnen als Terminkalendar heruntergeladen werden. (Men&uuml;punkt ICal Download)</li>"
											+ "<li style='padding-left:10px;'>NEU: Navigationsleiste kann eingeklappt werden.</li>"
											+ "<li style='padding-left:10px;'>NEU: Veranstaltungen k&ouml;nnen per Doppelklick hinzugef&uuml;gt / entfernt werden.</li>"
											+ "<li style='padding-left:10px;'>NEU: Bei &Auml;nderungen zu Ihrem abgespeicherten Plan werden jetzt sinnvolle Vorschl&auml;ge gemacht.</li>"
											+ "<li style='padding-left:10px;'>NEU: Kontrastreiche Men&uuml;s, sinnvollere Neuanordnung des Men&uuml;s.</li>"
											+ "<li style='padding-left:10px;'>NEU: Seitentitel auch als Titel des pdf-download.</li>"
											+ "<li style='padding-left:10px;'>NEU: Kleinere Texte bei den Einzelterminen.</li>"
											+ "<br/>"
											+ "<b>Version: 2.1.2:</b>"
											+ ""
											+ "<li style='padding-left:10px;'>NEU: MNI Style</li>"
											+ "<li style='padding-left:10px;'>GE&Auml;NDERT: PDF Download und PDF Dateiname bezieht sich auf den aktiven Tab</li>"
								});
								...
								handler : function() {
									Ext.Msg
											.confirm(
													...
													function(r) {
														if (r == 'yes') {
															var lessons = MySched.selectedSchedule
																	.getLectures();
															var toremove = [];
															for ( var i = 0; i < lessons.length; i++) {
																if ((lessons[i].data.type == "personal" && ((lessons[i].data.owner == MySched.Authorize.user && lessons[i].data.responsible == MySched.selectedSchedule.id) || MySched.Authorize
																		.isClassSemesterAuthor()))
																		|| MySched.selectedSchedule.id == "mySchedule") {
															...
															for ( var i = 0; i < toremove.length; i++) {
																if (MySched.selectedSchedule.id == "mySchedule") {
																	MySched.selectedSchedule
																			.removeLecture(MySched.selectedSchedule
																					.getLecture(toremove[i]));
																} else {
																	var tab = MySched.layout.tabpanel
																			.getComponent(MySched.Base.schedule
																					.getLecture(toremove[i]).data.responsible);
																	if (tab)
																		tab.mSchedule
																				.removeLecture(tab.mSchedule
																						.getLecture(toremove[i]));
																	MySched.selectedSchedule
																			.removeLecture(MySched.selectedSchedule
																					.getLecture(toremove[i]));
																...
																.append(
																				...
																				.getBody(),
																		{
																			...
																			src : _C('ajaxHandler')
																					...
																					+ MySched.Authorize.user
																					+ "&title="
																					+ encodeURIComponent(MySched.selectedSchedule.title
																							.replace(
																									/\s*\/\s*/g,
																									' '))
																					+ "&what=pdf&save=false&scheduletask=Download.schedule",
														...
														if (responseData['url'] != "false") {
															Ext.MessageBox
																	.show({
																		...
																		msg : '<strong style="font-weight:bold">Link</strong>:<br/>'
																				+ responseData['url']
																				+ '<br/>Wollen Sie den Terminkalendar ersetzen?',
																									...
																									src : _C('ajaxHandler')
																											...
																											+ encodeURIComponent(MySched.selectedSchedule.title
																													.replace(
																															/\s*\/\s*/g,
																															' '))
																											+ "&what=ics&save=true&scheduletask=Download.schedule",
																									style : 'display:none;z-index:10000;'
																								});
																				// Iframe
																				// wird
																									...
																									src : _C('ajaxHandler')
																											...
																											+ encodeURIComponent(MySched.selectedSchedule.title
																													.replace(
																															/\s*\/\s*/g,
																															' '))
																											+ "&what=ics&save=false&scheduletask=Download.schedule",
																									style : 'display:none;z-index:10000;'
																								});
																				// Iframe
																				// wird
																			...
																			src : _C('ajaxHandler')
																					...
																					+ encodeURIComponent(MySched.selectedSchedule.title
																							.replace(
																									/\s*\/\s*/g,
																									' '))
																					+ "&what=xls&save=false&scheduletask=Download.schedule",
																			style : 'display:none;z-index:10000;'
																		});
														// Iframe wird nach 2
														// Sec geloescht.
...
Ext.form.VTypes['ValidTime'] = function(arg, field) {
	if (field.id == "starttiid") {
		if (!Ext.getCmp('endtiid').getValue())
			return true;
		if (Ext.getCmp('starttiid').getValue() < Ext.getCmp('endtiid')
				.getValue()) {
			return true;
		}
		return false;
...
function newPEvent(pday, pstime, petime, title, teacher_name, clas_name, room_name,
	...
	if (l)
		...
		var lock = MySched.selectedSchedule.type;
	var titel = {
		layout : 'form',
		width : 550,
		labelAlign : 'top',
	...
	for ( var i = 0; i < MySched.Mapping.room.length; i++) {
		roomstore.push(new Array(MySched.Mapping.room.items[i].id,
				MySched.Mapping.room.items[i].name.replace(/^\s+/, '').replace(
						/\s+$/, '')));
	}
									...
									handler : function() {
										...
										if (titel && day && stime && etime) {
											...
											for ( var a = 0; a < rooms.length; a++) {
												...
												if (rooms[a] != "") {
													for ( var i = 0; i < MySched.Mapping.room.length; i++) {
														if (MySched.Mapping.room.items[i].name
																.replace(
																		/^\s+/,
																		'')
																.replace(
																...
																.replace(
																		...
																		'')) {
															if (!room
																	.contains(MySched.Mapping.room.items[i].id)) {
																if (room == "")
																	room = MySched.Mapping.room.items[i].id;
																else
																	room = room
																			+ " "
																			+ MySched.Mapping.room.items[i].id;
															}
															found = true;
															break;
														}
	...
	if (l) {
		...
		pwin
				.setTitle(MySchedLanguage.COM_THM_ORGANIZER_SCHEDULER_LESSON_CHANGE);
...
function blocktotime(block) {
	if (Ext.isNumber(block) && typeof block != "undefined"
			&& MySched.daytime[1] != null)
		return {
...
//									}
					...
					var elements = MySched.Mapping.getObjectField(type,
							nodeKey, "elements");
					if (elements === nodeKey)
						new mSchedule(nodeID, title).init(type, nodeKey).show();
					else
						new mSchedule(nodeID, title).init(type, elements)
								.show();
				}
			}
		},
		...
		setTreeData : function(data) {
			...
			var imgs = Ext.DomQuery.select(
					'img[class=x-tree-ec-icon x-tree-elbow-plus]',
					MySched.Tree.tree.body.dom);
			for ( var i = 0; i < imgs.length; i++) {
				imgs[i].alt = "collapsed";
			}
							...
							'Ext.grid.GridPanel',
							{
								...
								viewConfig : {
									...
									getRowClass : function(record, rowIndex, p,
											store) {
										if (this.showPreview) {
											p.body = '<p style="padding-left:25px; text-decoration:italic;">'
													+ record.data.subscribe_info
													+ '</p>';
