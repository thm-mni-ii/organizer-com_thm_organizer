...
class nusoap_client extends nusoap_base
{
     ...
     * constructor
     *
     * @param   array  $params  the parameters used for client configuration:
     *                          endpoint, wsdl, proxyhost, proxyport, proxyusername,
     *                          proxypassword, timeout, response_timeout, portName
     */
    public function nusoap_client($params)
    {
        ...
        if ($wsdl)
        {
            if (is_object($this->endpoint) AND (get_class($this->endpoint) == 'wsdl'))
            {
                $this->wsdl = $this->endpoint;
    ...
    function call($operation, $params = array(), $namespace = 'http://tempuri.org', $soapAction = '', $headers = false, $rpcParams = null, $style = 'rpc', $use = 'encoded')
    {
        ...
        if ($this->endpointType == 'wsdl' AND is_null($this->wsdl))
        {
            $this->loadWSDL();
            if ($this->getError())
                return false;
        ...
        if ($this->endpointType == 'wsdl' AND $opData = $this->getOperationData($operation))
        {
            ...
            // add ns to ns array
            if ($namespace != '' AND !isset($this->wsdl->namespaces[$namespace]))
            {
        ...
        else
        {
            ...
            // fault?
            if (is_array($return) AND isset($return['faultcode']))
            {
    ...
    private function checkWSDL()
    {
        ...
        // catch errors
        $errstr = $this->wsdl->getError();
        $soapOperations = $this->wsdl->getOperations($this->portName, 'soap');
        $soap12Operations = $this->wsdl->getOperations($this->portName, 'soap12');
        if (!empty($errstr))
        {
            $this->appendDebug($this->wsdl->getDebug());
            $this->wsdl->clearDebug();
        ...
        elseif (!empty($soapOperations))
        {
            $this->operations = $soapOperations;
            $this->appendDebug($this->wsdl->getDebug());
            $this->wsdl->clearDebug();
            $this->bindingType = 'soap';
            $this->debug('got ' . count($this->operations) . ' operations from wsdl ' . $this->wsdlFile . ' for binding type ' . $this->bindingType);
        }
        elseif (!empty($soap12Operations))
        {
            $this->operations = $soap12Operations;
            $this->appendDebug($this->wsdl->getDebug());
            $this->wsdl->clearDebug();
     ...
     * 
     * @return  array  data pertaining to the operation
     */
    ...
    function send($msg, $soapaction = '', $timeout = 0, $response_timeout = 30)
    {
        ...
        switch (true)
        {
            ...
            case preg_match('/^http/', $this->endpoint):
                ...
                if ($this->persistentConnection == true AND is_object($this->persistentConnection))
                {
    ...
    {
        $rand = rand();
        $evalStr = $this->_getProxyClassCode($rand);
        ...
        eval("\$proxy = new nusoap_proxy_$rand('');");
     ...
     * @access   private
     */
    function _getProxyClassCode($rand)
    {
        $this->debug("in getProxy endpointType=$this->endpointType");
        $this->appendDebug("wsdl=" . $this->varDump($this->wsdl));
        if ($this->endpointType != 'wsdl')
        {
            $errorString = 'A proxy can only be created for a WSDL client';
            $this->setError($errorString);
            return "echo \"$errorString\";";
        }
        if ($this->endpointType == 'wsdl' AND is_null($this->wsdl))
        {
            $this->loadWSDL();
        ...
        foreach ($this->operations as $operation => $opData)
        {
            if ($operation != '')
            {
                ...
                if (sizeof($opData['input']['parts']) > 0)
                {
                    $paramArray = array();
                    $paramStrArray = array();
                    $paramCommentArray = array();
                    foreach ($opData['input']['parts'] as $name => $type)
                    {
                        $paramArray[] = $name;
                        $paramStrArray[] = "'$name' => \$$name";
                        $paramCommentArray[] = "$type \$$name, ";
                    }
                    $paramStr = implode(', ', $paramArray);
                    $paramArrayStr = implode(', ', $paramStrArray);
                    $paramCommentStr = implode(', ', $paramCommentArray);
                }
                else
                {
                    $paramStr = '';
        ...
        $evalStr = 'class nusoap_proxy_' . $rand . ' extends nusoap_client {
        ' . $evalStr . '
    }';
        return $evalStr;
    }
    ...
    {
        $rand = rand();
        return $this->_getProxyClassCode($rand);
    }
     ...
     * Gets all cookies
     *
     * @return  array  all internal cookies
     */
