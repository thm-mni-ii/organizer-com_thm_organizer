 ...
 */
class Room_Overview extends FormModel
{
    const DAY = 1;
    ...
    protected function populateState()
    {
        ...
        $formData      = Input::getFormItems()->toArray();
        ...
        if (empty($formData)) {
            ...
            if (isset($formData['types'])) {
                $formData['types'] = ArrayHelper::toInteger($formData['types']);
            ...
            if (isset($formData['rooms'])) {
                $formData['rooms'] = ArrayHelper::toInteger($formData['rooms']);
    ...
    private function getDay($date)
    {
        ...
        if ($isSunday) {
            ...
            $getNext  = ($template == self::DAY and $isSunday);
            if ($getNext) {
