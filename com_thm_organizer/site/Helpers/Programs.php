...
class Programs
{
    ...
    {
        $dbo   = \Factory::getDbo();
        $table = new \Organizer\Tables\Plan_Programs($dbo);
    ...
    public static function getPlanResourceID($program)
    {
        ...
        $dbo               = \Factory::getDbo();
        $planResourceTable = new \Organizer\Tables\Plan_Programs($dbo);
    ...
    private static function getPlausibleData($gpuntisID)
    {
        ...
        if ($plausibleNumber) {
            ...
            if ($plausibleDegree) {
                $dbo            = \Factory::getDbo();
                $degreeTable    = new \Organizer\Tables\Degrees($dbo);
    ...
    {
        $dbo    = \Factory::getDbo();
        $table  = new \Organizer\Tables\Programs($dbo);
        $exists = $table->load($programData);
        if ($exists) {
            return $table->id;
        }
