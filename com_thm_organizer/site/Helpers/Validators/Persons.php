...
class Persons extends ResourceHelper implements UntisXMLValidator
{
    ...
    {
        $exists       = false;
        $person       = $model->schedule->persons->$untisID;
        $table        = self::getTable();
        $loadCriteria = [];
        ...
        $model->schedule->persons->$untisID->id = $table->id;
    ...
    {
        $model->schedule->persons = new stdClass;

        foreach ($xmlObject->teachers->children() as $node) {
     ...
     *
     * @param object &$model the validating schedule model
     * @param object &$node  the teacher node to be validated
     *
     * @return void
     */
    public static function validateIndividual(&$model, &$node)
    {
        ...
        $model->schedule->persons->$internalID = $person;
