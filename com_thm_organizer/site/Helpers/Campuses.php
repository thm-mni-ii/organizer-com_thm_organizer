...
class Campuses
{
    ...
    {
        $dbo = \Factory::getDbo();
        $table = new \Organizer\Tables\Campuses($dbo);
        $table->load($campusID);
    ...
    {
        $coordinates = preg_match('/\d{1,2}\.\d{6},[ ]*\d{1,2}\.\d{6}/', $input);
        if (empty($coordinates)) {
            $coordinates = self::getLocation($input);
        }
        ...
        $pin = '<a target="_blank" href="https://www.google.de/maps/place/' . $coordinates . '">';
