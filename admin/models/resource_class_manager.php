...
class THM_OrganizersModelResource_Class_Manager extends JModel
{
	 /* Items total
     * @var integer
     */
	...
	function _buildQuery()
	{
		...
		$filter_order		= $mainframe->getUserStateFromRequest( "$option.$view.filter_order",		'filter_order',		"#__thm_organizer_classes.id, #__thm_organizer_classes.name", 'string' );
		...
		if (!$filter_order) { $filter_order = '#__thm_organizer_classes.id, #__thm_organizer_classes.name'; }
		if (!$filter_order_Dir) {$filter_order_Dir = ''; }

		$orderby     = "\n ORDER BY $filter_order $filter_order_Dir";

	    $query="SELECT #__thm_organizer_classes.id, #__thm_organizer_classes.gpuntisID, #__thm_organizer_classes.name, #__users.name AS userName, CONCAT(#__thm_organizer_semesters.organization, ' - ', #__thm_organizer_semesters.semesterDesc) AS semesterName, #__thm_organizer_departments.name AS dptName " .
               "FROM #__thm_organizer_classes " .
               "INNER JOIN #__thm_organizer_departments " .
               "ON #__thm_organizer_classes.dptID = #__thm_organizer_departments.id " .
               "INNER JOIN #__users " .
               "ON #__thm_organizer_classes.manager = #__users.id " .
               "INNER JOIN #__thm_organizer_semesters " .
               "ON #__thm_organizer_classes.semester = #__thm_organizer_semesters.id " .
               "WHERE ";
		...
		$query.= ' LOWER(#__thm_organizer_classes.name) LIKE \'%'.$search.'%\' ';
	...
	function getTotal() {
 		...
 		if (empty($this->_total)) {
 		    ...
 		    $query = 'SELECT count(*) as anzahl FROM #__thm_organizer_classes';
			$db->setQuery($query);
