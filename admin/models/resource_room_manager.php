...
class THM_OrganizersModelResource_Room_Manager extends JModel
{
	 /* Items total
     * @var integer
     */
	...
	function _buildQuery()
	{
		...
		$filter_order		= $mainframe->getUserStateFromRequest( "$option.$view.filter_order",		'filter_order',		"#__thm_organizer_rooms.id, #__thm_organizer_rooms.name", 'string' );
		...
		if (!$filter_order) { $filter_order = '#__thm_organizer_rooms.id, #__thm_organizer_rooms.name'; }
		if (!$filter_order_Dir) {$filter_order_Dir = ''; }

		$orderby     = "\n ORDER BY $filter_order $filter_order_Dir";

	    $query="SELECT #__thm_organizer_rooms.id, #__thm_organizer_rooms.gpuntisID, #__thm_organizer_rooms.name, #__users.name AS userName, #__thm_organizer_rooms.capacity, #__thm_organizer_rooms.type, #__thm_organizer_departments.name AS dptName " .
               "FROM #__thm_organizer_rooms " .
               "INNER JOIN #__thm_organizer_departments " .
               "ON #__thm_organizer_rooms.dptID = #__thm_organizer_departments.id " .
               "INNER JOIN #__users " .
               "ON #__thm_organizer_rooms.manager = #__users.id " .
               "WHERE ";
		...
		$query.= ' LOWER(#__thm_organizer_rooms.name) LIKE \'%'.$search.'%\' ';
		$query.= ' OR LOWER(#__thm_organizer_rooms.type) LIKE \'%'.$search.'%\' ';
		$query.= ' OR LOWER(#__thm_organizer_rooms.manager) LIKE \'%'.$search.'%\' ';
	...
	function getTotal() {
 		...
 		if (empty($this->_total)) {
 		    ...
 		    $query = 'SELECT count(*) as anzahl FROM #__thm_organizer_rooms';
			$db->setQuery($query);
