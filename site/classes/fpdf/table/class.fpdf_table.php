...
*/
require_once(B."/fpdf/table/class.multicelltag.php");
...
function DrawTableLine($data, $header, $pDataType = 0){
	...
	for($i=0; $i < $this->tb_columns; $i++)
	{
        ...
        if (!isset($data[$i]['TEXT_STRLINES'])) $data[$i]['TEXT_STRLINES'] = $this->StringToLines($data[$i]['CELL_WIDTH'], utf8_decode($data[$i]['TEXT']));
...
function TableAddPage($header = true){
	...
	$this->SetDrawColor(32,0,0);
	$this->SetLineWidth(0.3);
	$this->SetFillColor(204,204,153);
//	$this->RoundedRect(15, 15, 180, 20, 5, 'DF', '1234');
	$this->SetXY(15,20);
	$this->SetLineWidth(0);
...
function MultiCellTable($w, $h, $txtData, $border=0, $align='J', $valign='T', $fill=0, $vh=0)
{
	...
	*/
	...
	$this->MultiCellTag($w, $h, $txtData, $b2, $align, 0, false);
...
function Footer() {
	...
	$this->Cell(60,10,'phpMyCamp v0.1',0,0,'L');
	$this->SetFont('Arial','B',8);
	$this->Cell(60,10,'Page '.$this->PageNo().'/{nb}',0,0,'C');
	$this->SetFont('Courier','',8);
	$this->Cell(60,10,'http://www.71eme.be',0,0,'R');
}
