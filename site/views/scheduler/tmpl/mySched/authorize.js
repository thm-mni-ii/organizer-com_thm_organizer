/**
 * Objekt zum Authentifizieren des Benutzers und
 * der Verwaltung dessen Rechten
 * @author thorsten
 */
MySched.Authorize = function () {
	...
	return {
		...
		/**
		 * Aendert die Rolle des aktuellen Users (zb. beim Anmelden von standard auf spiezielle)
		 * Hierbei wird automatisch der Baum neugeladen falls die Rolle sich aendert
		 * @param {Object} role
		 * @param {Object} rights
		 */
				...
				success: function (response, request) {
					if (response.responseText != "Permission Denied!") {
						...
						if (json.success) {
							...
							success.call(scope, json); // uebergebene Callback ausfuehren
						} else {
		...
		/**
		 * Ein Vorcheck, ob fuer einen bestimmten Typ ueberhaupt Berechtigungen vorliegen oder nicht
		 * gibt 'none' -> Keinerlei Rechte, 'part' -> Partielle Rechte oder 'full' -> Volle Rechte zurueck
		 * @param {Object} type
		 */
		...
		/**
		 * Zeigt das Authentifizierungsfenster an
